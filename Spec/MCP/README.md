...
2. `get_header_property_schemas`
3. `get_sheet_schema`
4. `get_sheet_rows`
5. `validate_column_type`
6. `get_current_sheet`
7. `get_current_selection`
8. `get_active_editor_context`
9. `create_workbook`
10. `create_sheet`
11. `patch_sheet_rows`
12. `patch_sheet_columns`

其中 `patch_sheet_rows` 与 `patch_sheet_columns` 当前通过“读取工作簿 -> 在 MCP bridge 内应用结构化补丁 -> 调用现有 workbook 保存接口回写”的方式工作，属于共享应用服务层落地前的过渡实现。
...
6. `lightydesign://editor/current-sheet`
   返回桌面端当前活动 Sheet 的摘要上下文。
7. `lightydesign://editor/current-selection`
...
| `get_active_editor_context` | 读取当前编辑器完整上下文 | 需消费桌面端上下文快照 | 包括工作区、Sheet、选区 |
...
5. AI 查询当前编辑状态时，应优先走“当前 Sheet / 当前选区”上下文接口，而不是再次全量读取整个工作区。
...
4. 实现只读 tools：`get_workspace_navigation`、`get_sheet_schema`、`get_sheet_rows`、`get_header_property_schemas`、`validate_column_type`、`get_current_sheet`、`get_current_selection`。

完成标准：

1. AI 可以在不打开前端源码的前提下理解工作区结构、当前活动表和当前选区。
2. 大表读取不会一次性返回全部行。
...
4. 对写操作增加 `dryRun` 支持。
5. 在 MCP Server 中暴露相应工具。

完成标准：

1. AI 可以新增行、更新行、删除行、追加列、修改列属性。
2. 错误信息能指出是哪一列或哪一行失败。
3. 冲突写入会被拒绝而不是静默覆盖。
...
3. 基于 `SampleWorkspace` 的集成测试，覆盖读表、改表、保存、导出。
4. 针对冲突版本、非法类型、非法引用和非法输出路径的失败测试。
