...
4. src/components/VirtualSheetTable.tsx
   虚拟滚动表格组件，负责表格渲染和单元格编辑控件切换。
5. src/components/ToastCenter.tsx
   通知中心组件，负责消息气泡栈和错误详情弹层渲染。
6. src/hooks/useEditorShortcuts.ts
   快捷键注册 Hook，负责把快捷键绑定到全局键盘事件。
7. src/hooks/useDesktopHostConnection.ts
   宿主连接 Hook，负责 DesktopHost 信息与健康状态轮询。
8. src/hooks/useToastCenter.ts
   通知中心 Hook，负责消息栈状态、自动消失和详情复制。
9. src/hooks/useWorkspaceEditor.ts
   工作区编辑会话 Hook，负责工作区加载、tabs、Sheet 加载、dirty state 和保存链路。
10. src/types/desktopApp.ts
