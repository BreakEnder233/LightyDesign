...
15. 已支持基于 FlowCharts 节点定义生成 `partial class` 骨架文件，并为模板内标准节点输出对应的标准 helper 绑定代码。
16. 已支持流程图标准节点的两类族模型首版生成：
	- `List.Add`、`Dictionary.Set` 这类 `generic` 绑定节点会生成泛型节点骨架。
	- `Arithmetic.Add`、`Comparison.Equal` 这类 `overload` 绑定节点会生成受控重载节点骨架。
17. 已支持按流程图实例生成 `Definition`、`Flow<TContext>` 和 `FlowChartRuntimeSupport.cs`。
18. 已支持流程图默认入口推导、显式入口覆盖，以及 `StepOnce`、`Step`、`RunToCompletion`、`RunUntilPaused`、`RunUntil`、`Resume` 等运行时骨架。
19. 已支持内置 `If`、`While`、`Pause`、`WaitUntil`、`PauseSeconds`、`List.ForEach`、`Dictionary.ForEach` 的首版运行时分派。
20. 已支持将 FlowChart 生成结果与 Workbook 生成结果共同汇总进同一个 `LDD.cs` 入口，并在增量或全量导出时保持二者共存。
21. 已通过测试验证生成出的 Workbook 代码、FlowChart 代码与 `LDD.cs` 入口能够一起编译通过。

## 当前尚未实现的业务能力

1. 尚未提供可配置的分片阈值或按估算代码体积的更细粒度拆分策略；当前仅支持代码内写死阈值的按行数分片。
2. 尚未支持 `Generated/Workbooks` 与 `Generated/FlowCharts` 双目录输出。
3. 尚未完全收敛 `Context<TExternalContext>` 的职责边界；当前流程图运行时的通用状态保存在生成出的 `Flow<TContext>` 上，这一方向目前保留。
4. 尚未把节点实例状态从通用 `FlowChartNodeState` 容器收敛为按节点生成的强类型状态槽；当前 `PauseSeconds` 仍通过通用节点状态保存累计等待时间。
5. 尚未为自定义有状态节点建立共享运行时契约；当前有状态运行时主要依赖内置控制/迭代节点的专用模板。
6. 尚未支持流程图标准节点的节点实例级类型推断与 flow 文件导出期签名收敛。
...
8. 超过阈值时的大表分片文件生成。
9. 缺失输出路径时的失败行为。
10. FlowChart `Definition`、`Flow<TContext>`、运行时辅助文件和标准节点 helper 生成。
11. FlowChart 导出写盘与 `LDD.cs` 对 Workbook / FlowChart 共存场景的重写行为。
12. 生成出的 Workbook 代码、FlowChart 代码与 `LDD.cs` 入口可一起编译通过。
13. `WaitUntil` / `PauseSeconds` 等待型控制节点的模板解析、runtime dispatch 和编译集成。
...
1. 下一阶段的目标是澄清 Flow 与 Context 的职责边界，继续保留 Flow 上的通用生命周期状态，并只在有必要时为图专属数据补强类型 Context。
2. 详细规划见 [FlowChartRuntimeContextPlan.md](FlowChartRuntimeContextPlan.md)。

## 当前状态结论

Generator 已经从工程骨架阶段进入可用的第一版实现阶段，能够围绕 Core 的统一协议模型生成 Workbook 与 FlowChart 两条代码生成链路，并通过统一的 `LDD.cs` 入口把它们汇总到同一套输出中。当前输出结构已经明确为“目标目录保留业务文件，所有生成结果集中写入其 `Generated` 子目录”，同时所有生成类型均使用 `partial`，Sheet 列成员以可读写属性形式输出，FlowChart 运行时骨架也已进入可编译、可导出的可用状态，内置等待型控制节点 `WaitUntil` / `PauseSeconds` 也已进入首版可用。后续工作的重点主要转向更细粒度的大表优化、更完整的 FlowChart 状态节点模板，以及更通用的运行时引用解析与节点契约机制。
