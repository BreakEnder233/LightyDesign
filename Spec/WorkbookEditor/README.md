...
1. [CompositeValueEditing.md](CompositeValueEditing.md)：定义 object / list / dictionary 等复合值的 JSON 编写、右键扩展编辑器和递归模态规则。
...
9. 已为 number 列接入专用 `NumericCellInput`，行内编辑不再退化为普通文本框。
10. 已为单元格右键菜单接入按类型的值编辑器入口，当前在精确单选时支持 number、reference、list、dictionary、object。
11. 已落地 `CellValueEditorDialog`，支持 number、reference、list、dictionary 的值编辑；其中 list / dictionary 支持结构化视角、JSON 视角和递归子模态。
12. object 已接入 JSON fallback 值编辑链路，但仍未具备 schema 驱动的字段级结构化编辑。
...
5. object 的 schema 驱动结构化编辑尚未落地，目前仍仅支持 JSON fallback；具体设计与后续目标见 [CompositeValueEditing.md](CompositeValueEditing.md)。
