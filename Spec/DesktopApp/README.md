...
11. 已接入基础快捷键系统，并支持撤销与恢复快捷键。
12. 已接入基于现有 workbook 保存接口的前端保存按钮。
...
1. 真实工作簿导航树。
2. 更完整的可编辑表格体验，例如更丰富的类型化编辑器、批量操作与更多快捷键。
3. 表头编辑器。
4. 导出按钮与宿主联动。
5. Excel 导入导出流程。
6. 更细粒度的保存与冲突处理体验。
...
DesktopApp 已经从“仅有桌面壳状态页”进入“可浏览真实工作区、使用虚拟滚动编辑表格、按列类型切换编辑器并按工作簿保存”的阶段。下一阶段应继续沿着现有 tabs、dirty state、undo/redo 和 save 链路推进更复杂的类型化编辑器、更细粒度保存与冲突处理，而不是在前端重建协议层。 
