...
public sealed class LightyWorkbookCodeGenerator
{
    ...
    private static GeneratedRowModel AnalyzeRow(LightyWorkspace workspace, LightySheet sheet, LightySheetRow row, int rowIndex, IReadOnlyList<GeneratedFieldModel> fields, IReadOnlyList<GeneratedFieldModel> primaryKeyFields, List<LightyGeneratedI18nEntry> i18nEntries)
    {
        ...
        foreach (var field in fields)
        {
            ...
            else if (field.TypeDescriptor.IsList)
            {
                ...
                if (elementDescriptor.IsLocalString)
                {
    ...
    private static string RenderLocalStringFile()
    {
        ...
        writer.AppendLine("using System;");
        writer.AppendLine("using System.Collections.Generic;");
        writer.AppendLine("using System.IO;");
        writer.AppendLine();
        ...
        writer.AppendLine();
        writer.AppendLine("public static event Action? OnLanguageChanged;");
        writer.AppendLine();
        ...
        writer.AppendLine("var manifestPath = Path.Combine(_basePath, _sourceLanguage, \"i18n_manifest.yaml\");");
        writer.AppendLine("if (!File.Exists(manifestPath))");
        writer.AppendLine("    throw new FileNotFoundException($\"本地化清单文件缺失：语言 '{_sourceLanguage}' 的清单文件不存在，期望路径: {manifestPath}\");");
        writer.AppendLine("var manifest = File.ReadAllText(manifestPath);");
        ...
        writer.AppendLine("{");
        writer.Indent();
        writer.AppendLine("var yamlPath = Path.Combine(_basePath, language, wb.File);");
        writer.AppendLine("if (!File.Exists(yamlPath))");
        writer.AppendLine("    throw new FileNotFoundException($\"本地化文件缺失：语言 '{language}' 缺少工作簿 '{wb.Name}' 的映射文件，期望路径: {yamlPath}\");");
        writer.AppendLine("var yaml = File.ReadAllText(yamlPath);");
