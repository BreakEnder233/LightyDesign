...
19. 已支持内置 `If`、`While`、`Pause`、`List.ForEach`、`Dictionary.ForEach` 的首版运行时分派。
...
4. 尚未把节点实例状态从通用 `FlowChartNodeState` 容器收敛为按节点生成的强类型状态槽，也尚未覆盖 `PauseSeconds` 等等待型节点的状态模板。
...
Generator 已经从工程骨架阶段进入可用的第一版实现阶段，能够围绕 Core 的统一协议模型生成 Workbook 与 FlowChart 两条代码生成链路，并通过统一的 `LDD.cs` 入口把它们汇总到同一套输出中。当前输出结构已经明确为“目标目录保留业务文件，所有生成结果集中写入其 `Generated` 子目录”，同时所有生成类型均使用 `partial`，Sheet 列成员以可读写属性形式输出，FlowChart 运行时骨架也已进入可编译、可导出的可用状态。后续工作的重点主要转向更细粒度的大表优化、更完整的 FlowChart 状态节点模板，以及更通用的运行时引用解析与节点契约机制。
