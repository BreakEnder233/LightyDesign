...
8. 工作区初始化能力已创建 `Workbooks`、`FlowCharts/Nodes` 和 `FlowCharts/Files` 默认目录。

## 当前尚未实现的工作

1. SampleWorkspace 和相关测试样例尚未迁移到新结构。
2. DesktopApp 尚未消费新的 FlowChart 资产导航与保存接口。
3. 尚未制作正式的工作区模板目录，用于承载完整工作区结构、默认表格、默认控制节点、默认集合访问节点和默认简单计算节点。
4. 新建工作区流程尚未改为直接复制工作区模板内容。
...
Workspace 子系统的目录协议已经冻结，且 Core 与 DesktopHost 已完成第一批落地。下一步重点是迁移样例工作区、接入 DesktopApp/FlowChartEditor，并补齐模板、校验和代码生成链路。
