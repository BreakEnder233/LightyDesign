...
33. 已接入首版 `FlowChartEditor`，在桌面壳中提供 Workbook / FlowChart 全局模式切换，并保持两个编辑器目录并列。
34. 已落地流程图导航、节点库、画布和 Inspector 的三段式流程图编辑界面。
35. 已支持在流程图画布中拖拽节点、直接发起端口连线，并区分流程端口与计算端口的视觉和连线样式。
36. 已支持流程图文件的前端本地结构校验与保存，保存前会阻止计算回路等显式错误。
37. 已补齐流程图画布的缩放、框选、多选、批量拖拽、复制、剪切、粘贴与小地图，并把对应快捷键接入桌面壳统一快捷键系统。
...
2. 尚未实现跨编辑器共享标签页模型。
3. 尚未把流程图引用字段的跳转与标签编排接入桌面壳。
4. 尚未实现流程图节点定义资产编辑器与更完整的流程图库管理界面。
5. 尚未补齐流程图画布的批量属性编辑、对齐分布和自动排版等更复杂的批量编辑体验。
...
6. src/flowchart-editor/
   当前 FlowChart 专属前端目录，承载流程图导航、编辑状态、画布与 Inspector。
7. src/workbook-editor/components/VirtualSheetTable.tsx
   虚拟滚动表格组件，负责表格渲染和单元格编辑控件切换。
8. src/components/ToastCenter.tsx
   通知中心组件，负责消息气泡栈和错误详情弹层渲染。
9. src/hooks/useEditorShortcuts.ts
   快捷键注册 Hook，负责把快捷键绑定到全局键盘事件。
10. src/hooks/useDesktopHostConnection.ts
   宿主连接 Hook，负责 DesktopHost 信息与健康状态轮询。
11. src/hooks/useToastCenter.ts
   通知中心 Hook，负责消息栈状态、自动消失和详情复制。
12. src/workbook-editor/hooks/useWorkspaceEditor.ts
   当前主要服务表格编辑会话。后续需要继续按桌面壳共享状态与 WorkbookEditor 专属状态重新拆分。
13. src/workbook-editor/types/desktopApp.ts
   当前 Workbook 编辑器使用的共享类型、轻量模型、validation schema DTO 和纯工具函数。
14. src/styles.css
   当前桌面界面样式，包括侧边栏、tabs、表格区、表头编辑器和通知层。
15. src/workbook-editor/components/ColumnEditorDialog.tsx
   列编辑弹窗，负责表头属性输入、Type 校验、validation schema 侧边说明区和规则预校验。
16. src/flowchart-editor/components/FlowChartCanvas.tsx
   流程图画布组件，负责节点卡片渲染、端口命中、框选、多选拖拽、缩放和小地图呈现。
17. src/flowchart-editor/hooks/useFlowChartEditor.ts
   流程图编辑状态 Hook，负责流程图导航、文件加载、节点定义缓存、选择模型、剪贴板、保存和本地结构校验。
...
DesktopApp 已经具备稳定的桌面壳基础设施，并已同时承载 WorkbookEditor 与 FlowChartEditor 两个并列子系统。下一阶段的重点不再是“接入流程图编辑器”，而是继续推进共享标签模型、跨编辑器跳转，以及流程图高级批量编辑与节点定义资产编辑器。
